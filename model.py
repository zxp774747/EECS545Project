...
def discriminator(images, reuse, args):
  ...
  with tf.name_scope('discriminator'):
    with slim.arg_scope([slim.conv2d], activation_fn=None, kernel_size=[5, 5], stride=2, reuse=reuse):
      ...
      D = slim.batch_norm(D, activation_fn=tf.nn.relu, is_training=is_training, reuse=reuse, scope='discriminator_batchnorm1')

      D = slim.conv2d(images, 2 * args.d_feature_dim, scope='discriminator_conv2')
      D = slim.batch_norm(D, activation_fn=tf.nn.relu, is_training=is_training, reuse=reuse, scope='discriminator_batchnorm2')

      D = slim.conv2d(images, 4 * args.d_feature_dim, scope='discriminator_conv3')
      D = slim.batch_norm(D, activation_fn=tf.nn.relu, is_training=is_training, reuse=reuse, scope='discriminator_batchnorm3')

      D = slim.conv2d(images, 8 * args.d_feature_dim, scope='discriminator_conv4')
      D = slim.batch_norm(D, activation_fn=tf.nn.relu, is_training=is_training, reuse=reuse, scope='discriminator_batchnorm4')
...
def DCGAN(real_images, args):
  ...
  X = tf.mul(tf.sub(real_images, 0.5), 2)  # scale to [-1, 1]
  prob_real, logits_real = discriminator(X, False, args)
  fake_images = generator(z, args)
  fake_image_summary = tf.image_summary('fake_images', tf.mul(tf.add(fake_images, 1), 255.0 / 2), max_images=args.batch_size)
  ...
  return G_loss, D_loss, G_loss_summary, D_loss_summary, fake_image_summary