...
def parseArgs():
  ...
  parser.add_argument('mode', choices=['train', 'sample'], type=str, help='training mode or sampling mode')
  ...
  parser.add_argument('--n_threads', type=int, default=2, help='the number of data-loading threads')
  parser.add_argument('--learning_rate', type=float, default=0.0002)
  parser.add_argument('--beta1', type=float, default=0.5)

  args = parser.parse_args()

  if args.mode == 'sample' and not args.checkpoint:
    print '--checkpoint is compulsory when mdoe == sample'
    sys.exit(1)
  ...
  args.img_depth = 3 if args.dataset == 'stl10' else 32
